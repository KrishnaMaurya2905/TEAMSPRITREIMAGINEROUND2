...
//           height={1080}
//           width={1920}
//           className="absolute h-full max-sm:h-[80vh] w-full left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 "
//         />
//       </div>
//     </div>
//   );
...
const CanvasImageSequence = () => {
  ...
  useEffect(() => {
    ...
    const urls = new Array(frameCount)
      ...
      .map(
        (_, i) =>
          `/src/assets/scene${(i + 1).toString().padStart(5, "0")}.png`
      );
    ...
    <div className="w-full h-full">
      <div className="relative dad w-full h-screen  bg-black">
        <canvas
          ...
          height={1440}
          width={2560}
          className="absolute h-full w-full left-1/2 top-1/2 max-sm:h-[60vh] transform -translate-x-1/2 -translate-y-1/2  max-sm:w-[180vw]"
        />
      </div>
    </div>
  );
